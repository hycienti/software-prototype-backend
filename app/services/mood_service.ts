...
import MoodRepository from '#repositories/mood_repository'
...
export default class MoodService {
  /**
   * Create a mood entry, update achievements and invalidate AI cache.
   */
  async create(
    userId: number,
    payload: {
    ...
    await aiInsightsService.invalidateCache(userId, 'mood')
    return mood
  }

  /**
   * List mood entries with pagination and optional filters.
   */
  async listForUser(
    userId: number,
    params: { page?: number; limit?: number; startDate?: Date; endDate?: Date; mood?: string }
  ) {
    const page = params.page ?? 1
    const limit = Math.min(params.limit ?? 20, 100)
    return moodRepository.listByUserIdPaginated(userId, page, limit, {
  ...
  /**
   * Get a single mood by id and userId.
   */
  async getById(userId: number, id: number): Promise<Mood> {
    return moodRepository.findByIdAndUserId(id, userId)
  }
  ...
  async update(
    ...
    const mood = await moodRepository.findByIdAndUserId(id, userId)
    return moodRepository.update(mood, payload)
  }

  /**
   * Delete a mood entry and recalculate achievements.
   */
  async destroy(userId: number, id: number): Promise<void> {
    const mood = await moodRepository.findByIdAndUserId(id, userId)
    await moodRepository.delete(mood)
    await this.checkAndUpdateAchievements(userId)
  }
  ...
  async getStreak(userId: number): Promise<{ streak: number; lastEntryDate: string | null }> {
    ...
    const lastEntry = await moodRepository.getLastEntryByUserId(userId)
    return {
      streak,
  ...
  /**
   * Get mood insights and analytics (optionally with AI insights).
   */
  async getMoodInsights(
    ...
    try {
      const allMoods = await moodRepository.listAllByUserId(userId)
  ...
  private detectPatterns(
    ...
    moods.forEach((m) => {
      const dayName = m.entryDate.toFormat('cccc')
      if (!dayOfWeekCounts[dayName]) dayOfWeekCounts[dayName] = {}
  ...
  async calculateStreak(userId: number): Promise<number> {
    try {
      const moods = await moodRepository.listRecentForUser(userId, 365)
  ...
  async checkAndUpdateAchievements(userId: number): Promise<Achievement[]> {
    try {
      const streak = await this.calculateStreak(userId)
      const entryCount = await moodRepository.countByUserId(userId)
